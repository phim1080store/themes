function replaceEpisodeSlug(newSlug) {
...
function getNextPrevEpisode(isNext) {
    episodeList = episodeList.filter((value, index, self) => index === self.findIndex((e) => e.slug === value.slug && e.server === value.server && e.movie_id === value.movie_id))
...
function getAudioUrl() {
    const href = document.querySelector('li.on > a')?.getAttribute('href')
    ...
    const filteredLinks = allLinks.filter((a) => {
        const dataId = a.getAttribute('href')
    ...
    const selectorFromServer = filteredLinks.map((a, index) => {
        const episodeDiv = a.closest('div[id^="episode-"]')
            ...
            if (data.fatal) {
                switch (data.type) {
                    case Hls.ErrorTypes.NETWORK_ERROR:
                        art.notice.show = 'Mất mạng, đang thử kết nối lại...'
                        art.hls.startLoad()
                        art.hls.recoverMediaError()
                        break
                    case Hls.ErrorTypes.MEDIA_ERROR:
                        art.notice.show = 'Lỗi media, đang khôi phục...'
                        art.hls.recoverMediaError()
                        break
                    default:
                        art.notice.show = 'Lỗi nghiêm trọng, dừng phát...'
                        art.hls.destroy()
                        break
                }
            }
    ...
    if (type == 'm3u8') {
        ...
        let settings = [
            ...
            },
            {
                ...
                onSelect: function (item) {
                    const selected = autoData.selectorFromServer.find((i) => i.value === item.value)
        ...
        new MutationObserver(function (mutationsList) {
            mutationsList.forEach((mutation) => {
                if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                    const classList = artVideoPlayer.classList
        ...
        new MutationObserver((mutationsList) => {
            const artNoticeInner = document.querySelector('.art-notice-inner')
            mutationsList.forEach((mutation) => {
                if (mutation.type === 'characterData' || mutation.type === 'childList') {
                    const text = artNoticeInner.textContent.trim()
